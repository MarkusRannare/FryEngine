...
namespace
{
	...
	public:
		...
		~MallocAllocator() {
			...
			// destroyed.
			CHECK_ALWAYS(_total_allocated == 0);
		}
	...
	public:
		...
		virtual void *allocate(uint32_t size, uint32_t align) {
			...
			if( in_use( p ) )
			{
				// @TODO: Make some kind of mark if we enter here so that we can notify
				// the creator of the allocator that we have overflowed the scratch-allocator
		...
		virtual void deallocate(void *p) {
			...
			Header *h = header(p);
			CHECK_ALWAYS((h->size & 0x80000000u) == 0);
	...
	{
		static const int ALLOCATOR_MEMORY = sizeof(MallocAllocator) + sizeof(MallocAllocator) + sizeof(ScratchAllocator);
		char buffer[ALLOCATOR_MEMORY];

		MallocAllocator *default_allocator;
		MallocAllocator *default_renderthread_allocator;
		ScratchAllocator *default_scratch_allocator;

		MemoryGlobals() : default_allocator(0), default_renderthread_allocator(0), default_scratch_allocator(0) {}
	};
...
namespace foundation
{
	namespace memory_globals
	{
		void init( uint32_t temporary_memory )
		{
			...
			_memory_globals.default_renderthread_allocator = new(p) MallocAllocator();
			p += sizeof(MallocAllocator);
		...
		{
			return *_memory_globals.default_renderthread_allocator;
		}
		...
		void shutdown()
		{
			...
			_memory_globals.default_renderthread_allocator->~MallocAllocator();
